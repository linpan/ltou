...
class BaseMenuModel(BaseModel, abc.ABC):
    ...
    @abc.abstractmethod
    def ask(self, context: "Context") -> Optional["Context"]:
        pass

    @abc.abstractmethod
    def need_ask(self, context: "Context") -> bool:
        pass

    def after_ask(self, context: "Context") -> "Context":
...
class SingularMenuModel(BaseMenuModel):
    ...
    cli_name: Optional[str]
    description: str
    before_ask_fun: Optional[Callable[["Context"], Optional[MenuEntry]]]
    after_ask_fun: Optional[Callable[["Context", "SingularMenuModel"], "Context"]]
    ...
    def ask(self, context: "Context") -> Optional["Context"]:
        ...
        if chosen_entry == SKIP_ENTRY:
            return
        # add self.code to context dict
        setattr(context, self.code, chosen_entry.code)

        return context

    def after_ask(self, context: "Context") -> Optional["Context"]:
...
class MultiselectMenuModel(BaseMenuModel):
    before_ask: Optional[Callable[["Context"], Optional[list[MenuEntry]]]]
...
class Context(UserDict, MutableMapping):
    """
    context is a dictionary that can be used as a context manager.
    collection user interaction is done through this context manager.
    """
